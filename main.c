...
/* Changed so that it returns the write value instead of just returning zero which is not helpful in catching erros*/

int cdc_send(uint8_t *buf, uint16_t len) {
  return write(serial_port, buf, len);
}
int cdc_recv(uint8_t *buf, uint16_t len) {
  return read(serial_port, buf, len);
}
int cdc_putchar(uint8_t x) { return cdc_send(&x, 1); }
int cdc_getchar(uint8_t *x) { return cdc_recv(x, 1); }
